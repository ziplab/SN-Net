...
from timm.loss import JsdCrossEntropy, SoftTargetCrossEntropy, BinaryCrossEntropy, \
    LabelSmoothingCrossEntropy, StitchDistillationLoss
...
fh = logging.FileHandler(f'./logs/log-{datetime.now().strftime("%Y%m%d-%H%M%S")}.txt', 'a+')
...
group.add_argument('--checkpoint-hist', type=int, default=5, metavar='N',
...
group.add_argument('--teacher-path', default='', type=str,
...
def initiailize_stitching_layer(args, model, data_config, num_aug_splits, device):
    temp_dataset_train = create_dataset(
        args.dataset,
        root=args.data_dir,
    ...
    if num_aug_splits > 1:
        temp_dataset_train = AugMixDataset(temp_dataset_train, num_splits=num_aug_splits)

    # create data loaders w/ augmentation pipeiine
    train_interpolation = args.train_interpolation
    if args.no_aug or not train_interpolation:
        train_interpolation = data_config['interpolation']
    temp_loader_train = create_loader(
        temp_dataset_train,
    ...
    for samples, targets in temp_loader_train:
...
def main():
    ...
    with open(args.stitch_config, 'r') as f:
        stitch_configs = json.load(f)

    anchor_models = []
    cnn_to_vit = False
    for name in stitch_configs['anchors']:
        if 'swin' in name:
            cnn_to_vit = True
        model = create_model(
            name,
    ...
    # create teacher model for SN-Net training time distillation
    print(f"Creating teacher model")
    ...
    teacher_model.cuda()
    teacher_model.eval()
    train_loss_fn = StitchDistillationLoss(train_loss_fn, teacher_model)
...
def validate(
    ...
    output_dir = os.path.join(save_dir, f'stitches_res.txt')
    ...
    for cfg_id in range(last_cfg_id+1, num_configs):
        ...
        _logger.info(f'------------- Evaluting stitch config {cfg_id}/{num_configs} -------------')
        ...
        log_stats['acc1'] = top1_m.avg
        log_stats['acc5'] = top5_m.avg
