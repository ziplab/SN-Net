...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, start=None, end=None, extract_feat=False):
        features = {}
        if extract_feat:
...
class SwinTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.depths = depths
        self.stage_dims = [32 * n_heads for n_heads in num_heads]
    ...
    # for initializing stitching layers
    def extract_stage_features(self, x):
        ...
        for i, layer in enumerate(self.layers):
            x, stage_features = layer(x, extract_feat=True)
            features[i] = stage_features
        return features
    ...
    def forward_until(self, x, stage_id, blk_id):
        ...
        for i, layer in enumerate(self.layers):
            x = layer(x, end=blk_id if i == stage_id else None)
            if i == stage_id:
                break
        return x
    ...
    def forward_from(self, x, stage_id, blk_id):
        for i, layer in enumerate(self.layers):
            if i < stage_id:
                continue
            x = layer(x, start=blk_id if i == stage_id else None)
