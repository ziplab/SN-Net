...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        self.layers = layers
        self.depths = layers
        self.stage_dims = [512 * block.expansion // 8, 512 * block.expansion // 4, 512 * block.expansion // 2, 512 * block.expansion]
    ...
    def extract_layer_features(self, x, layer):
        features = {}
        features[-1] = x.detach().cpu()
        for i, blk in enumerate(layer):
            x = blk(x)
            features[i] = x.detach().cpu()
        return x, features
    ...
    def extract_stage_features(self, x):
        ...
        x, stage_feat1 = self.extract_layer_features(x, self.layer1)
        x, stage_feat2 = self.extract_layer_features(x, self.layer2)
        x, stage_feat3 = self.extract_layer_features(x, self.layer3)
        x, stage_feat4 = self.extract_layer_features(x, self.layer4)
        features[0] = stage_feat1
        features[1] = stage_feat2
        features[2] = stage_feat3
        features[3] = stage_feat4
        return features

    def forward_until(self, x, stage_id=-1, blk_id=-1):
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.act1(x)
        x = self.maxpool(x)
        ...
        for s_id, layer in enumerate(layers):
            for b_id, blk in enumerate(layer):
                x = blk(x)
                if s_id == stage_id and b_id == blk_id:
                    break
            if s_id == stage_id:
                break
        return x

    def forward_from(self, x, stage_id=-1, blk_id=-1):
        layers = [self.layer1, self.layer2, self.layer3, self.layer4]
        for s_id, layer in enumerate(layers):
            if s_id < stage_id:
                continue
...
@register_model
def stitch_resnet_18_34_50(pretrained=False, anchor_models=None, cnn_to_vit=False, **kwargs):
    model = AssembleResNets(anchor_models, cnn_to_vit=cnn_to_vit)
    return model