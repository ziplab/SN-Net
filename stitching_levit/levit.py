...
class LeViT(torch.nn.Module):
    ...
    def __init__(self, img_size=224,
        ...
        # find donwsampling layers
        self.subsample_ids = []
        for i, mod in enumerate(self.blocks):
            if mod.is_subsample_layer:
                self.subsample_ids.append(i)
    ...
    def extract_block_features(self, x):
        ...
        features = {}
        subsample_ids = self.subsample_ids + [len(self.blocks)]
        start_idx = -2

        for idx, sub_idx in enumerate(subsample_ids):
            stage_feats = {}
            stage_feats[-1] = x.detach()
            for i, blk in enumerate(self.blocks[start_idx+2:sub_idx]):
                x = blk(x)
                if i % 2 == 1: # only save block outputs
                    stage_feats[i//2] = x.detach()
            features[idx] = stage_feats
            start_idx = sub_idx
            x = self.blocks[sub_idx:sub_idx + 2](x)
    ...
    def forward_until(self, x, stage_id, blk_id):
        ...
        idx = stage_id * 10 + (blk_id + 1) * 2

        for blk in self.blocks[:idx]:
            x = blk(x)
        return x

    def forward_from(self, x, stage_id, blk_id):
        idx = stage_id * 10 + blk_id * 2
