...
def build_optimizer(config, model, simmim=False, is_pretrain=False):
    ...
    if simmim:
        ...
        parameters = set_weight_decay_stitch(model, skip, skip_keywords, config.TRAIN.BASE_LR_SCALE, config.TRAIN.STITCH_LR_SCALE)
...
def set_weight_decay_stitch(model, skip_list=(), skip_keywords=(), base_lr_scale=1.0, stitch_lr_scale=1.0, ):
    has_decay = []
    no_decay = []
    ...
    for name, param in model.named_parameters():
        ...
        if len(param.shape) == 1 or name.endswith(".bias") or (name in skip_list) or \
            ...
            has_decay.append(param)
    return [{'params': has_decay, 'lr_scale': base_lr_scale},
            {'params': no_decay, 'weight_decay': 0., 'lr_scale': base_lr_scale},
            {'params': stitching_groups, 'lr_scale': stitch_lr_scale}]


def set_weight_decay_stitch_v2(model, skip_list=(), skip_keywords=(), base_lr_scale=1.0, stitch_lr_scale=1.0):
    has_decay = []
    no_decay = []
    ...
    return [{'params': has_decay, 'lr_scale': base_lr_scale},
            {'params': no_decay, 'weight_decay': 0., 'lr_scale': base_lr_scale},
            {'params': stitching_weights, 'lr_scale': stitch_lr_scale},
